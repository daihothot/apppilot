...
/** Executes and validates one command through the Unity Pipeline transport. */
export class UnityPipelineTool {
  readonly id = "unity-pipeline";
    ...
    try {
      ...
      const operationArgs = operation === "version"
        ...
        : operation === "status" || operation === "list"
          ...
          : ["--json", "--non-interactive", "command", "--timeout", String(timeoutSeconds), operation];
      const command = await this.hostCommands.run({
      ...
      if (command.exitCode !== 0) {
        return {
          success: false,
          operation,
          status: command.exitCode === 124 ? "timed_out" : "failed",
          error: command.stderr.trim() || `Unity Pipeline ${operation} failed.`,
        };
      }
      const parsed = operation === "version"
        ? parseVersionResult(command.stdout)
        : parseJsonResult(operation, command.stdout);
      return {
...
function parseVersionResult(stdout: string): ParsedUnityPipelineResult {
  ...
  return version
    ...
    : { success: false, error: "Unity Pipeline returned an empty version." };
}
...
function parseJsonResult(operation: string, stdout: string): ParsedUnityPipelineResult {
  ...
  try {
    ...
    return { success: false, error: `Unity Pipeline result is not valid JSON: ${String(error)}` };
  }
  const expectedCommand = operation.startsWith("editor_") ? `command ${operation}` : operation;
  if (!isRecord(value)
    ...
    return { success: false, error: `Unity Pipeline result does not match operation ${operation}.` };
  }
  ...
  if (!isRecord(data)) {
    return { success: false, errors, warnings, error: `Unity Pipeline ${operation} data must be an object.` };
  }
  if (operation === "status") {
    if (typeof data.count !== "number"
      ...
      return { success: false, errors, warnings, error: "Unity Pipeline status data must contain count and instances." };
    }
    ...
    for (const instance of data.instances) {
      if (!isRecord(instance)
        ...
        return { success: false, errors, warnings, error: "Unity Pipeline status instances must contain version and state." };
      }
    ...
    if (typeof data.count !== "number"
      ...
      return { success: false, errors, warnings, error: "Unity Pipeline list data must contain count and tools." };
    }
    const commands: UnityPipelineJsonValue[] = [];
    for (const tool of data.tools) {
      if (!isRecord(tool) || typeof tool.name !== "string" || tool.name.length === 0) {
        return { success: false, errors, warnings, error: "Unity Pipeline list tools must contain non-empty names." };
      }
    ...
    if (data.command !== operation || typeof data.success !== "boolean") {
      return { success: false, errors, warnings, error: `Unity Pipeline ${operation} data does not match the editor command result contract.` };
    }
    parsedSuccess = value.success && data.success;
    if (operation !== "editor_status") {
      if (typeof data.result !== "string" || data.result.length === 0) {
        return { success: false, errors, warnings, error: `Unity Pipeline ${operation} result must be a non-empty message.` };
      }
      return {
    ...
    if (!isRecord(data.result)) {
      return { success: false, errors, warnings, error: `Unity Pipeline ${operation} result does not contain the required Editor state.` };
    }
